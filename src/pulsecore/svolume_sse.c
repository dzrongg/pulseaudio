...
static void svolume_sse_unit_test_aux(int sample_type, void (*sample_func)(int16_t *samples, int32_t *volumes, unsigned channels, unsigned length)) {
    ...
    for (i = 0; i < SAMPLES; i++)
        fail_unless(samples[i] == samples_ref[i], NULL);
}

void svolume_sse_unit_test() {
    svolume_sse_unit_test_aux(PA_SAMPLE_S16NE, pa_volume_s16ne_sse2);
    svolume_sse_unit_test_aux(PA_SAMPLE_S16RE, pa_volume_s16re_sse2);
}
