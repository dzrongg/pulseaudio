...
static void svolume_mmx_unit_test_aux(int sample_type, void (*sample_func) (int16_t *samples, int32_t *volumes, unsigned channels, unsigned length)) {
    ...
    func = pa_get_volume_func(sample_type);
    ...
    sample_func(samples, volumes, CHANNELS, sizeof(samples));

    fail_unless(memcmp(samples_ref, samples, sizeof(samples)) == 0, NULL);
}

void svolume_mmx_unit_test() {
    svolume_mmx_unit_test_aux(PA_SAMPLE_S16NE, pa_volume_s16ne_mmx);
    svolume_mmx_unit_test_aux(PA_SAMPLE_S16RE, pa_volume_s16re_mmx);
}
